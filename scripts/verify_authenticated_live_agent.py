...
async def _verify_async(args: argparse.Namespace) -> dict:
    ...
    async with async_playwright() as playwright:
        ...
        selection_version_count = await page.locator('[data-live-message-selection-version="v26"]').count()
        record(
            "live-message-selection-version-current",
            selection_version_count == 1,
            "Agent is running the v26 message-selection bundle that forces the installed app shell forward, preserves manual live-message selection across feed refreshes, resets stale preview caches, and never embeds live preview frames.",
