...
class LiveReviewPanelFrontendTests(unittest.TestCase):
    ...
    def test_live_review_side_panel_renders_clickthrough_and_replay_controls(self) -> None:
        ...
        self.assertIn('key={workbenchState?.missionId || currentProjectLabel || "agent-run"}', shell)
        self.assertIn('key={workbenchState?.missionId || currentProjectLabel || "workbench"}', shell)
        self.assertIn('key={`${workbenchState?.missionId || "mission"}:${previewFrameUrl}`}', shell)
