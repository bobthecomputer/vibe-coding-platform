...
class DesktopUiContractTests(unittest.TestCase):
    ...
    def test_provider_ecosystem_is_visible_in_runtime_drawer(self) -> None:
        ...
        self.assertIn("Provider route decision matrix", provider_panel)
        self.assertIn("provider-route-decision-matrix", provider_panel)
        self.assertIn("providerRouteDecision", provider_panel)
        self.assertIn("routeDecision", provider_panel)
        self.assertIn("Source-backed model capability catalog", provider_panel)
        self.assertIn("provider-model-catalog", provider_panel)
        self.assertIn("modelCatalog", provider_panel)
        self.assertIn("defaultChangeAllowed=false", provider_panel)
        self.assertIn("provider-model-catalog-policy", provider_panel)
        ...
        self.assertIn(".provider-route-decision-matrix", styles)
        self.assertIn(".provider-route-decision-card", styles)
        self.assertIn(".provider-route-decision-proof", styles)
        self.assertIn(".provider-model-catalog", styles)
        self.assertIn(".provider-model-catalog-policy", styles)
        self.assertIn(".provider-model-catalog-card", styles)
    ...
    def test_current_shell_mounts_image_and_voice_surfaces(self) -> None:
        ...
        self.assertIn('if (source.startsWith("/")) return source;', image_studio)
        ...
        self.assertIn("ui_self_repair_planner", image_studio)
        self.assertIn("self_repair_verifier", image_studio)
        ...
        self.assertIn("Image operation timeline", image_studio)
        self.assertIn("Operation timeline", image_studio)
        ...
        self.assertIn("Sample only - synthetic green screen", image_studio)
        ...
        self.assertIn('"controls rail"', image_css)
        self.assertIn(".image-studio-local-preview-badge", image_css)
        self.assertIn(".image-studio-advanced-disclosure", image_css)
        self.assertIn("image-studio-breakdown-in", image_css)
        self.assertIn(".image-studio-run-result", image_css)
        self.assertIn(".image-studio-operation-timeline", image_css)
        self.assertIn(".image-studio-operation-timeline-head", image_css)
        self.assertIn("--image-studio-timeline-index", image_css)
        ...
        self.assertTrue((ROOT / "web" / "public" / "image-studio" / "generated-coastal-retreat.png").exists())
        self.assertIn("Local preview backdrop", image_state)
        self.assertIn("Generated coastal retreat image", image_state)
        self.assertIn("/image-studio/generated-coastal-retreat.png", image_state)
        self.assertIn("codex_image_playground_live_review_reference_20260511T071327Z", image_state)
        self.assertIn('type: "image"', image_state)
        ...
        self.assertIn('"name": "image_vision_breakdown"', skills)
        self.assertIn('"name": "ui_self_repair_planner"', skills)
        self.assertIn('"name": "self_repair_verifier"', skills)
        self.assertIn('"preferred_model": "glm-5.2"', skills)
        proof_script = (ROOT / "scripts" / "image_vision_self_repair_proof.py").read_text(encoding="utf-8")
        self.assertIn("PREFERRED_ROUTE", proof_script)
        self.assertIn("send_agent_chat_command", proof_script)
        self.assertIn("vision_breakdown.json", proof_script)
