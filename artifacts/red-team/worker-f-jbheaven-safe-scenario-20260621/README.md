...
The probe families are mapped to current AI-risk review categories, including
OWASP LLM01 prompt injection, LLM02 sensitive information disclosure, LLM06
excessive agency, LLM07 system prompt leakage, and LLM09 misinformation. The
NIST AI 600-1 mapping is used only as a documentation scaffold for safe
measurement and does not imply a live model evaluation occurred.
...
- Keep route promotion blocked until a supervised Hermes/OpenClaw lab run
  attaches browser proof, visible transcript proof, and human review notes.
...
- `browser-proof.json`: browser proof manifest generated by the visual smoke.
- `redteam-proof-desktop.png` and `redteam-proof-mobile.png`: current Fluxio
  shell screenshots of the controlled proof board.
